class StreamerApp {
   ...
   async loadMovies() {
       ...
       try {
           ...
           if (stored) {
               ...
               this.movies = this.getDemoMovies();
   ...
   createCard(movie) {
       ...
       return `
           <div class="flex-shrink-0 w-36 md:w-44 movie-card" onclick="${hasContent ? `app.playMovie('${movie.id}')` : ''}">
               <div class="card-glow rounded-xl"></div>
   ...
   async addDemoContent() {
       const demoMovies = [
           { id: 'demo-' + Date.now(), title: "Attack on Titan", category: "Anime", year: 2013, duration: "24 min", rating: 9.0, description: "Humans battle giant humanoid Titans.", poster: "https://cdn.myanimelist.net/images/anime/10/47347.jpg", videoUrl: "x8z1a2b", videoType: "dailymotion", views: 0, timestamp: Date.now() },
           { id: 'demo-' + (Date.now()+1), title: "Mo Dao Zu Shi", category: "Donghua", year: 2018, duration: "24 min", rating: 8.5, description: "A cultivator returns to life 13 years later.", poster: "https://cdn.myanimelist.net/images/anime/1258/93893.jpg", videoUrl: "x6z2a3c", videoType: "dailymotion", views: 0, timestamp: Date.now() - 1000 }
       ];
       try {
           this.movies.push(...demoMovies);
           localStorage.setItem('streamer_movies', JSON.stringify(this.movies));
   ...
   setupEventListeners() {
       document.addEventListener('click', (e) => {
           ...
           if (!e.target.closest('.ai-subtitle-panel') && !e.target.closest('[onclick*="toggleAISubtitles"]')) {
